...
const MANGOJUICE_VERSION = require(join(__dirname, "../package.json")).version;
const PACKAGES = fs.readdirSync(PACKAGES_DIR);

const lernaJSON = require(join(__dirname, "../lerna.json"));
lernaJSON.version = MANGOJUICE_VERSION;
fs.writeFileSync(
...
function updateDeps(deps, pkgJSON) {
  ...
  for (const dep in deps) {
    if (PACKAGES.indexOf(dep) > -1) {
      if (deps[dep] !== MANGOJUICE_VERSION) {
        res = true;
        deps[dep] = MANGOJUICE_VERSION;
        console.log(
...
for (let i = 0; i < PACKAGES.length; i += 1) {
  ...
  if (pathStat.isDirectory()) {
    ...
    if (pkgJSON.version !== MANGOJUICE_VERSION) {
      failed = true;
      pkgJSON.version = MANGOJUICE_VERSION;
      console.log(
